...
def ask_ai(prompt: str, system: str = SYSTEM_PROMPT) -> str:
    """Single entry point for all AI calls. Uses Groq (14,400 req/day free)."""
...
class MonthlyView(APIView):
    ...
    def get(self, request):
        ...
        income_map = {item['month_trunc']: item['total'] for item in income_qs}
        expense_map = {item['month_trunc']: item['total'] for item in expense_qs}

        all_months = sorted(set(list(income_map.keys()) + list(expense_map.keys())), reverse=True)[:6]
        ...
        data = [
            {
                ...
                'net': income_map.get(m, Decimal('0')) - expense_map.get(m, Decimal('0')),
            }
...
class ParseExpenseView(APIView):
    """Parse bank SMS or receipt text into structured expense data."""
    permission_classes = (IsAuthenticated,)
...
class MonthlySummaryView(APIView):
    """AI-generated monthly financial summary — cached per user per day in Redis."""
    permission_classes = (IsAuthenticated,)
    ...
    def get(self, request):
        ...
        # Serve from cache if available (reset daily)
...
Bu hakyky sanlara esaslanyp 2-3 sözlem jemleýji baha ber. Anyk sanlary mention et. Höweslendiriji we amaly maslahat ber."""
        ...
        try:
            text = ask_ai(prompt)
            cache.set(cache_key, text, 60 * 60 * 6)  # 6-hour cache
